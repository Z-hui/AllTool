...
+ (void)printfView:(UIView *)View
{
    if (View.subviews.count>0) {
        ...
        XHZLog(@"tag=%ld,View=%@",(long)View.tag, View);
    }
}
...
//        //        XHZLog(@"再次压缩:%d", [result length] / 1024);
//    }
//    return result;
}
...
+ (void)deleteFileFor:(NSString *)path
{
    ...
    if ([manager fileExistsAtPath:path]) {
        ...
        if (error) {
            XHZLog(@"remove File : %@",error.localizedDescription);
        }
    }
}
...
+ (void)getWANIPAddressWithCompletion:(void(^)(NSString *IPAddress))completion
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (error) {
            XHZLog(@"Failed to get WAN IP Address!(%@)", error.localizedDescription);
        } else {
...
+ (NSDictionary *)currentWiFiSSID
{
    ...
    //XHZLog(@"Supported interfaces: %@", ifs);
    for (NSString *ifnam in ifs) {
        NSDictionary *info = (__bridge_transfer id)CNCopyCurrentNetworkInfo((__bridge CFStringRef)ifnam);
        //XHZLog(@"dici：%@",[info  allKeys]);
        if (info && [info count]) {
...
- (NSDictionary *)convertToDic
{
    ...
    if (error) {
        XHZLog(@"字符串转换成字典失败,error : %@",error.localizedDescription);
    }
...
- (NSString *)getFilePath
{
    ...
    if (![NSFileManager fileExistsAtPath:documentsDirectory]) {
        ...
        if (error) {
            XHZLog(@"路径创建失败:%@",error.localizedDescription);
        }
    }
...
- (NSString *)convertToString
{
    ...
    if (error) {
        XHZLog(@"NSDictionary转NSString出错,%@",error.localizedDescription);
    }
...
+ (BOOL)removeItemAtPath:(NSString *)path
{
    ...
    if (error) {
        XHZLog(@"删除文件 : %@",error.localizedDescription);
    }
    return result;
}
